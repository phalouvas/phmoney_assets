...
import ReportsIncomeExpenseCashFlow from "@/components/reports/ReportsIncomeExpenseCashFlow.vue";
</script>
...
<template>
  <ReportLayout title="Cash Flow" :extra_params="extra_params">
    <template #settings>
      <div class="flex gap-2">
        <div>
          <form-label for="levels" value="Account Display Length" />
          <select
            id="levels"
    ...
    <div class="bg-white shadow mt-4 prose max-w-none">
      <ReportsIncomeExpenseCashFlow
        v-if="store.props.currency && !store.processing"
      />
    </div>
  </ReportLayout>
</template>
